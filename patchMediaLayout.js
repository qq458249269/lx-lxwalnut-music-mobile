...
public class VisualizerAudioProcessor extends BaseAudioProcessor {
  ...
  public void queueInput(ByteBuffer inputData) {
    // 先 tap 数据算频谱(用 duplicate,不消耗原 buffer);FFT 出错不影响播放
    if (sEnabled) {
      try {
    ...
    try {
      ...
      // 透传失败:清空输出避免卡死
      try { replaceOutputBuffer(0).flip(); } catch (Exception ignore) {}
    }
  }
...
try {
  // 2) MusicManager.java:renderersFactory 保持默认 factory（不挂 AudioProcessor）
  // AudioProcessor 挂载在本设备反复导致播放异常，已放弃。VisualizerAudioProcessor.java
  // 保留生成（供 VisualizerBarView 引用），但不挂到 ExoPlayer，播放稳定优先。
